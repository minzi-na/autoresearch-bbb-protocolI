...
**하이퍼파라미터는 baseline으로 완전 고정**하고, 아키텍처 구조 변경만으로 **ROC-AUC**, **MCC**를 최대화한다.

판단 기준: **internal test ROC-AUC** (scaffold split, 10 seeds mean) — keep/discard 결정
참고 지표: **external ROC-AUC**, **holdout ROC-AUC** (동일 10개 모델로 평가, soft voting ensemble) — generalization 검증, 결정에는 사용 안 함
...
**평가 방식**
- `roc_auc_scaffold`: internal test ROC-AUC scaffold split 10 seeds mean → keep/discard 기준
- `roc_auc_external`: 동일 모델로 external remaining 전체 평가 10-seeds soft voting ensemble
- `roc_auc_holdout`: 동일 모델로 merged holdout 전체 평가 10-seeds soft voting ensemble
...
6. **Initialize `bbb_results_<combo_abbr>_v2.tsv`** with just the header row.
7. **Confirm and go**.
...
**필드 설명**:
- `roc_auc_scaffold`, `mcc_scaffold` — keep/discard 판단 기준 (10-seed mean)
- `f1_scaffold`, `acc_scaffold` — internal scaffold test 보조 지표 (10-seed mean)
- `roc_auc_external`, `roc_auc_holdout` — generalization 검증 보조 지표 (10-seed soft voting ensemble)
- `mcc_holdout`, `f1_holdout`, `acc_holdout` — holdout calibration 분석용 보조 지표 (Phase 1에서는 참고만, keep 판단 미사용)

Extract key metrics:
```bash
grep "^roc_auc_\|^mcc_scaffold" bbb_run_combo<N>.log
```

## Logging results

콤보별로 별도 TSV 파일에 기록한다: `bbb_results_combo1_v2.tsv`
...
2. `roc_s`       — `roc_auc_scaffold` (10 seeds mean), use 0.000000 for crashes
3. `mcc_s`       — `mcc_scaffold` (10 seeds mean), use 0.000000 for crashes
4. `roc_ext`     — `roc_auc_external` (10 seeds soft voting ensemble), use 0.000000 for crashes
5. `roc_holdout` — `roc_auc_holdout` (10 seeds soft voting ensemble), use 0.000000 for crashes
...
| 콤보 | 실행 명령 | 결과 TSV |
|------|-----------|---------|
| combo1 (`maccs+avalon+rdkit+mole`) | `python bbb_train.py > bbb_run_combo1.log 2>&1` | `bbb_results_combo1_v2.tsv` |
| combo2 (`ecfp+maccs+avalon+tt+rdkit+scage1+mole`) | `python bbb_train.py > bbb_run_combo2.log 2>&1` | `bbb_results_combo2_v2.tsv` |
| combo3 (`ecfp+maccs+avalon+tt+rdkit+mole`) | `python bbb_train.py > bbb_run_combo3.log 2>&1` | `bbb_results_combo3_v2.tsv` |
...
1. `bbb_results_<combo>_v2.tsv`와 git 상태를 보고 지금까지 시도한 내용을 파악한다.
