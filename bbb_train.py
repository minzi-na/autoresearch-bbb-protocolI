...
class MultiModalGMLPFromFlat(nn.Module):
    ...
    def forward(self, x):
        ...
        # fp tokens attend to embed tokens (LN-normalized queries and keys)
        fp_t = X0[:, :self._n_fp, :]
        em_t = X0[:, self._n_fp:, :]
        d = X0.shape[-1]
        fp_n = F.layer_norm(fp_t, [d])  # normalize before computing similarities
        em_n = F.layer_norm(em_t, [d])
        ca_attn = torch.softmax(fp_n @ em_n.transpose(-1, -2) / (d ** 0.5), dim=-1)  # (B, n_fp, n_em)
        fp_cross = ca_attn @ em_t                                                       # values raw
