...
- **`roc_ext`**: external set 예측확률을 10-seed soft voting ensemble한 ROC-AUC
- **`roc_holdout`**: holdout set 예측확률을 10-seed soft voting ensemble한 ROC-AUC
...
2. **confirmation phase**
   - top-k trial 선택
   - `roc_s`: 10-seed scaffold test ROC-AUC mean으로 재평가
   - `roc_ext`, `roc_holdout`: 10-seed soft voting ensemble로 재평가
...
3. `bbb_optuna.py`를 준비한다.
   ...
   - reevaluation: `roc_s`는 10-seed scaffold test mean, `roc_ext`/`roc_holdout`은 10-seed soft voting ensemble
...
3. `best_ext`
   ...
   - 10-seed soft voting ensemble 기준
4. `best_holdout`
...
2. 해당 setting의 `roc_s` = internal scaffold test ROC-AUC의 10-seed mean
3. 해당 setting의 `roc_ext` / `roc_holdout` = 10-seed soft voting ensemble 일반화 성능
